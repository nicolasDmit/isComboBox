...
@Connect(IsComboBox.class)
public class IsComboBoxConnector extends AbstractComponentConnector
    ...
    public void updateFromUIDL(UIDL uidl, ApplicationConnection client) {
        ...
        boolean openByClick = uidl
                .hasAttribute(ComboBoxConstants.ATTR_OPEN_BY_CLICK)
                && uidl.getBooleanAttribute(
        ...
        List<IsComboBoxWidget.FilterSelectSuggestion> newSuggestions = new ArrayList<IsComboBoxWidget.FilterSelectSuggestion>();

        for (final Iterator<?> i = options.getChildIterator(); i.hasNext(); ) {
            final UIDL optionUidl = (UIDL) i.next();
            final IsComboBoxWidget.FilterSelectSuggestion suggestion = getWidget().new FilterSelectSuggestion(
                    optionUidl);
        ...
        if (suggestionsChanged) {
            ...
            for (IsComboBoxWidget.FilterSelectSuggestion suggestion : newSuggestions) {
        ...
        if ((getWidget().waitingForFilteringResponse && getWidget().lastFilter
            ...
            if (!getWidget().popupOpenerClicked
                    && getWidget().selectPopupItemWhenResponseIsReceived != IsComboBoxWidget.Select.NONE) {
    ...
    private void navigateItemAfterPageChange() {
        if (getWidget().selectPopupItemWhenResponseIsReceived == IsComboBoxWidget.Select.LAST) {
    ...
    private void performSelection(String selectedKey) {
        // some item selected
        for (IsComboBoxWidget.FilterSelectSuggestion suggestion : getWidget().currentSuggestions) {
    ...
    @Override
    public IsComboBoxWidget getWidget() {
        return (IsComboBoxWidget) super.getWidget();
    }
    ...
    @Override
    public void layout() {
        IsComboBoxWidget widget = getWidget();
        if (widget.initDone) {
