...
public class IsComboBoxWidget extends Composite
    ...
    public class FilterSelectSuggestion implements SuggestOracle.Suggestion, Command {
         ...
         * Constructor
         *
         * @param uidl The UIDL recieved from the server
         */
     ...
     */
    private class IsMouseWheeler extends IsJsniMousewheelHandler {

        public IsMouseWheeler() {
            super(IsComboBoxWidget.this);
        }
    ...
    static class IsJsniUtil {
        ...
        public static void moveScrollFromEvent(final Widget widget,
            ...
            if (!Double.isNaN(deltaY)) {
                IsComboBoxWidget filterSelect = (IsComboBoxWidget) widget;
    ...
    public class SuggestionPopup extends VOverlay
         ...
         */
        public final IsComboBoxWidget.SuggestionMenu menu;
        ...
        private final IsMouseWheeler isMouseWheeler = new IsMouseWheeler();
        ...
        SuggestionPopup() {
            ...
            setOwner(IsComboBoxWidget.this);
            menu = new IsComboBoxWidget.SuggestionMenu();
            setWidget(menu);
        ...
        protected void onLoad() {
            ...
            if (pageLength > 0) {
                isMouseWheeler.attachMousewheelListener(getElement());
            }
        }

        @Override
        protected void onUnload() {
            isMouseWheeler.detachMousewheelListener(getElement());
            super.onUnload();
        }
         ...
         *
         * @param currentSuggestions The filtered suggestions
         * @param currentPage        The current page number
         * @param totalSuggestions   The total amount of suggestions
         */
        public void showSuggestions(
                final Collection<IsComboBoxWidget.FilterSelectSuggestion> currentSuggestions,
             ...
             */
            final IsComboBoxWidget.SuggestionPopup popup = this;
            Scheduler.get().scheduleDeferred(new Scheduler.ScheduledCommand() {
                ...
                public void execute() {
                    ...
                    final int x = IsComboBoxWidget.this.getAbsoluteLeft();
        ...
        private void setNextButtonActive(boolean active) {
            ...
            if (active) {
                DOM.sinkEvents(down, Event.ONCLICK);
                down.setClassName(
                        IsComboBoxWidget.this.getStylePrimaryName() + "-nextpage");
            } else {
                DOM.sinkEvents(down, 0);
                down.setClassName(IsComboBoxWidget.this.getStylePrimaryName()
                        + "-nextpage-off");
            }
        }
        ...
        private void setPrevButtonActive(boolean active) {
            ...
            if (active) {
                DOM.sinkEvents(up, Event.ONCLICK);
                up.setClassName(
                        IsComboBoxWidget.this.getStylePrimaryName() + "-prevpage");
            } else {
                DOM.sinkEvents(up, 0);
                up.setClassName(IsComboBoxWidget.this.getStylePrimaryName()
                        + "-prevpage-off");
            }
        ...
        private boolean selectItem(final MenuItem newSelectedItem) {
            ...
            // Set the icon.
            IsComboBoxWidget.FilterSelectSuggestion suggestion = (IsComboBoxWidget.FilterSelectSuggestion) newSelectedItem
                    .getCommand();
         ...
         */
        private IsComboBoxWidget.SuggestionPopup.LazyPageScroller lazyPageScroller = new IsComboBoxWidget.SuggestionPopup.LazyPageScroller();
         ...
         *
         * @param paging Should the paging be turned on?
         */
        ...
        public void setPosition(int offsetWidth, int offsetHeight) {
            ...
            final int textInputHeight = IsComboBoxWidget.this.getOffsetHeight();
            ...
            if (offsetWidth + left > Window.getClientWidth()) {
                left = IsComboBoxWidget.this.getAbsoluteLeft()
                        + IsComboBoxWidget.this.getOffsetWidth() - offsetWidth;
                if (left < 0) {
                    left = 0;
        ...
        private void setTdWidth(Node parent, int width) {
            for (int i = 0; i < parent.getChildCount(); i++) {
                ...
                if ("td".equals(child.getNodeName().toLowerCase())) {
                    ((com.google.gwt.dom.client.Element) child).getStyle().setWidth(width, Style.Unit.PX);
                } else {
         ...
         *
         * @param uidl           UIDL for the whole combo box
         * @param componentState shared state of the combo box
         */
        public void updateStyleNames(UIDL uidl,
                                     AbstractComponentState componentState) {
            debug("VFS.SP: updateStyleNames()");
            setStyleName(
                    IsComboBoxWidget.this.getStylePrimaryName() + "-suggestpopup");
            menu.setStyleName(
                    IsComboBoxWidget.this.getStylePrimaryName() + "-suggestmenu");
            status.setClassName(
                    IsComboBoxWidget.this.getStylePrimaryName() + "-status");
    ...
    public class SuggestionMenu extends MenuBar
         ...
         *
         * @param suggestions The suggestions to be rendered in the menu
         */
        public void setSuggestions(
                Collection<IsComboBoxWidget.FilterSelectSuggestion> suggestions) {
            if (enableDebug) {
                debug("VFS.SM: setSuggestions(" + suggestions + ")");
            }

            clearItems();
            final Iterator<IsComboBoxWidget.FilterSelectSuggestion> it = suggestions.iterator();
        ...
        public String getSubPartName(
            ...
            com.google.gwt.dom.client.Element menuItemRoot = subElement;
        ...
        /**
         * @deprecated use {@link VFilterSelect.SuggestionPopup#selectFirstItem()} instead.
         */
        @Deprecated
        public void selectFirstItem() {
        ...
        /**
         * @deprecated use {@link VFilterSelect.SuggestionPopup#selectLastItem()} instead.
         */
        @Deprecated
        public void selectLastItem() {
     ...
     * For internal use only. May be removed or replaced in the future.
     */
    public final IsComboBoxWidget.SuggestionPopup suggestionPopup;
    ...
    private class IconWidget extends Widget {
        ...
        IconWidget(Icon icon) {
            ...
            addDomHandler(IsComboBoxWidget.this, ClickEvent.getType());
        }
    }

    private IsComboBoxWidget.IconWidget selectedItemIcon;
     ...
     * For internal use only. May be removed or replaced in the future.
     */
    public final List<IsComboBoxWidget.FilterSelectSuggestion> currentSuggestions = new ArrayList<>();
    ...
    private static double getMarginBorderPaddingWidth(com.google.gwt.dom.client.Element element) {
    ...
    protected TextBox createTextBox() {
        return new IsComboBoxWidget.FilterSelectTextBox();
    }
     ...
     * @since 7.1.5
     */
    protected IsComboBoxWidget.SuggestionPopup createSuggestionPopup() {
        return new IsComboBoxWidget.SuggestionPopup();
    }
     ...
     * filter
     *
     * @param page The page which items are to be filtered
     */
     ...
     * @param filter The filter to apply to the components
     */
     ...
     *
     * @param page      The page to filter
     * @param filter    The filter to apply to the options
     * @param immediate Whether to send the options request immediately
     */
     ...
     * For internal use only. May be removed or replaced in the future.
     *
     * @param text The text the text box should contain.
     */
    ...
    public void setSelectedItemIcon(String iconUri) {
        ...
        if (iconUri == null || iconUri.length() == 0) {
            ...
            IsComboBoxWidget.IconWidget newIcon = new IsComboBoxWidget.IconWidget(client.getIcon(iconUri));
     ...
     *
     * @param suggestion The suggestion that just got selected.
     */
     ...
     *
     * @param icon1 the first widget
     * @param icon2 the second widget
     * @return <code>true</code> if they show the same icon, <code>false</code>
     * otherwise
     */
    private static boolean iconEquals(IsComboBoxWidget.IconWidget icon1, IsComboBoxWidget.IconWidget icon2) {
        if (icon1 == null) {
            return icon2 == null;
        } else if (icon2 == null) {
            return false;
     ...
     *
     * @param event The KeyDownEvent of the key
     */
    ...
    private void selectPrevPage() {
        if (currentPage > 0) {
            ...
            selectPopupItemWhenResponseIsReceived = IsComboBoxWidget.Select.LAST;
        }
    }
    ...
    private void selectNextPage() {
        if (hasNextPage()) {
            filterOptions(currentPage + 1, lastFilter);
            selectPopupItemWhenResponseIsReceived = IsComboBoxWidget.Select.FIRST;
        }
    }
     ...
     *
     * @param event The KeyUpEvent of the key depressed
     */
    ...
    public void updateSuggestionPopupMinWidth() {
        ...
        for (IsComboBoxWidget.FilterSelectSuggestion suggestion : currentSuggestions) {
    ...
    private void afterUpdateClientVariables() {
        ...
        selectPopupItemWhenResponseIsReceived = IsComboBoxWidget.Select.NONE;
    }
     ...
     *
     * @param selectedCaption the caption of selected item
     * @since 7.7
     */
